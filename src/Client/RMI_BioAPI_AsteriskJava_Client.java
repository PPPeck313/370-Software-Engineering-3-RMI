//SERVER PORT IS BEING SOUGHT
//Naming.lookup("rmi://" + host_ip + "/RMI_BioAPI_AsteriskJava");
//PORT VALUE DEFAULTS TO 1099- REGISTRY, AND BOUND NAME IS SEARCHED FOR
...
public class RMI_BioAPI_AsteriskJava_Client {
    ...
    {
    	try {   		    	    
    	    Registry registry = LocateRegistry.getRegistry(host_ip, 1099);//MANUAL LOOKUP VS NAMING
    		RMI_BioAPI_AsteriskJava_Interface service = (RMI_BioAPI_AsteriskJava_Interface) registry.lookup("RMI_BioAPI_AsteriskJava");
    		
    		service.RPC_FileRead(Service_UID, srcFileName, socket_ip, socket_port, remote_fileName); //str
    	} catch (Exception e) {
